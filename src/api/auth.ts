    ...
    static async connectAsDefaultUser() {
        if (!defaultUserAuthVAJ) {
            ...
            if (!userName || !userEmail || !userPassword || !jwtSecret) {
                Error("Missing required environment variables: DEFAULT_USER_NAME, DEFAULT_USER_EMAIL, DEFAULT_USER_PASSWORD, or JWT_SECRET.");
            }
            ...
            if (foundUser) {
                ...
                const { 
                    ...
                    await pgPool.query(
                        ...
                        [user.name, user.email, user.password, user.systemAuthentication, user.systemRole]
                    )
                ).rows?.[0] || null;
                ...
                const { 
                    ...
                    await pgPool.query(
                        ...
                        [
                            ...
                            refreshToken.expiresAt!.toISOString().replace("T", " ").replace("Z", "")
                        ]
                    )
                )?.rows?.[0];
