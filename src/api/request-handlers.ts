...
// export const getPgClientHandler = async function (
//   req: ExpectedRequest,
//   res: ExpectedResponse,
//   next: NextFunction
// ) {
//   try {
//     const pgClient: Client | null | undefined = await getPgClient();
//     if (!pgClient) {
...
//     const orderBy: OrderByField[] | undefined = req.orderBy;
//     const where: WhereCondition[] | undefined = req.where;

//     // Format all the records from the orderBy param as strings
//     let orderByStrings: string[] = [];
//     if (orderBy) {
//       orderByStrings = orderBy.map(
//         ({ field, direction }) => `${field} ${direction.toUpperCase()}`
//       );
//     }

//     // Format all the records from the where param as strings
//     let whereConditionStrings: string[] = [];
//     if (where) {
//       whereConditionStrings = where.map(
//         ({ field, operator, value }) =>
//           `${field} ${operator} ${!isNaN(Number(value)) ? value : `'${value}'`}`
//       );
//     }
...
//         SELECT *
//         FROM shop.${clsName}
//         ${where ? `WHERE ${whereConditionStrings.join(" AND ")}` : ""}
//         ${orderBy ? `ORDER BY ${orderByStrings.join(", ")}` : ""}
