...
async function findUserByEmail(email: string) {
  ...
  if (users.length > 1)
    throw new DataInconsistencyError("MULTIPLE_USERS_WITH_SAME_EMAIL");
  else if (users.length === 0)
    throw new BadRequestError("EMAIL_IS_NOT_ASSIGNED_TO_USER");
  return users[0];
}
...
export async function loginUser(event: LoginEvent): Promise<LoginResult> {
  ...
  if (inputUser.password !== user?.password)
    throw new AuthenticationError(`INCORRECT_USER_PASSWORD`);
