...
export class File extends ShopFile {
  ...
  get bucketObj(): Bucket | null {
    return this.bucket ? storage.findBucket(this.bucket) : null;
  }
...
class StorageClient {
  ...
  private async getAuthorization() {
    ...
    const expirationTimestamp = this.authorization
      ? this.authorization.applicationKeyExpirationTimestamp
      : null;
    ...
    try {
      ...
      const headers = {
        Authorization:
          "Basic " +
          Buffer.from(`${applicationKeyId}:${applicationKey}`).toString(
            "base64",
          ),
      };
  ...
  async uploadFile(file: File) {
    ...
    const uploadUrl = await this.getUploadUrl(file.bucketObj!);
