...
function runRedisContainer() {
  ...
  if (!redisPort || !redisInsightsPort || !redisHost || !redisPassword) {
    LOGGER.error(
      "Missing required environment variables: REDIS_PORT, REDIS_INSIGHTS_PORT, REDIS_HOST, or REDIS_PASSWORD.",
    );
    process.exit(1);
  }
  ...
  try {
    ...
    LOGGER.info(
      `Redis stack container "${CONTAINER_NAME}" is running on ${redisHost}:${redisPort}.`,
    );
    LOGGER.info(
      `Redis insights container "${CONTAINER_NAME}" is running on ${redisHost}:${redisInsightsPort}.`,
    );
  } catch (error) {
